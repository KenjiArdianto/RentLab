    ...
    class PembayaranController extends Controller
    {
        ...
        public function createCheckoutInvoice(CreateCheckoutInvoiceRequest $request) // <-- 3. Gunakan CreateCheckoutInvoiceRequest
        {
            ...
            try {
                ...
                ->log("Created new payment and invoice for external_id '{$payment->external_id}'");
                ...
                \activity('admin_invoice_created')
                ->causedBy(Auth::user())
                ->performedOn($payment)
                ->withProperties([
                ...
                ->log("Xendit invoice created for external_id '{$payment->external_id}'");
                ...
                ->log("Payment creation failed for user_id {$user->id}");
        ...
        public function handleWebhook(Request $request)
        {
            ...
            if (!isset($payload['external_id'], $payload['status'])) {
                ...
                \activity('admin_webhook_invalid_payload')
                ->withProperties([
                    'payload' => $payload,
                    'ip' => $request->ip(),
                    'user_agent' => $request->userAgent(),
                ])
                ->log("Webhook rejected: missing required payload fields");
            ...
            if (!$payment) {
                ...
                \activity('admin_webhook_invalid_payment')
                ->withProperties([
                ...
                ->log("Webhook failed: payment with external_id '{$externalId}' not found");
            ...
            if ($payment->status === 'PAID' && $paymentStatus === 'PAID') {
                ...
                \activity('admin_webhook_received')
                ->performedOn($payment)
                ->withProperties([
                    'external_id' => $externalId,
                    'status' => $paymentStatus,
                    'payment_id' => $payment->id,
                    'updated_transactions' => $updatedRows ?? 0,
                    'payload' => $payload,
                    'ip' => $request->ip(),
                    'user_agent' => $request->userAgent(),
                ])
                ->log("Webhook processed for external_id '{$externalId}' with status '{$paymentStatus}'");
